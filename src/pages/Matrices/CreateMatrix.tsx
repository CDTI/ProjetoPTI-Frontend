...
export default function CreateMatrix() {
  const [ insertedDisciplines, setInsertedDisciplines ] = useState<TDisciplineMatrix[]>();
  ...
  async function handleStartUpload(e: any) {
    ...
    if(file) {  
      const fileName = file.name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[_]/g, "")
      data.append('file', file, fileName)
      await MatrixController.getInstance().uploadMatrix(data).then(res => {
    ...
    <div className="w-full h-full px-8">
      <header className="flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-x-4">
          <button onClick={() => document.getElementById("inputfile")?.click()} className="w-[200px] bg-blue-500 mt-6 h-9 rounded-sm" >
            <h1 className="text-xl font-bold text-white ">Upload</h1>
            <input onChange={handleUploadFile} id="inputfile"  className="mt-4" hidden type="file" accept=".csv" />
          </button>
        </div>
      </header>
      <main className="w-full h-full px-8 mt-4">
        {(!file && !isInserting) && <p className="text-xl">Faça o upload de uma matriz curricular para começar.</p>}
        {insertedDisciplines && (file && insertedDisciplines?.length < 1) && (
          <div className="flex flex-col">
            <p className="text-xl">Matriz a ser inserida: <span className="font-bold">{file.name}</span>, deseja começar?</p>
            <button onClick={handleStartUpload} className="w-[300px] bg-blue-500 mt-6 h-9 rounded-sm" >
        ...
        {insertedDisciplines && (insertedDisciplines.length > 0) && !isInserting && (
          <div className="mt-4">
            <p className="mb-12">Essas foram as disciplinas inseridas ao curso de <span className="font-bold">{courseOfMatrix}.</span></p>
            <DisciplinesTable 
